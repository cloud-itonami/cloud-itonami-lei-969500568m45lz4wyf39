...
- `scripts/verify-facts.cljk` — re-fetches every source `facts.edn` cites and fails if the live registry no longer agrees (`kbb --backend sci scripts/verify-facts.cljk`; exit 0 = match, 1 = drift/broken citation, 3 = could not check). Vendored from `com-junkawasaki/root`.
